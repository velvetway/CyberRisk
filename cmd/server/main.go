...
func main() {
	...
	if snap, err := bdu.Open(cfg.BDUSnapshotPath); err != nil {
		log.Printf("bdu snapshot at %q unavailable: %v — autodetect disabled", cfg.BDUSnapshotPath, err)
	} else {
		bduSnap = snap
		defer bduSnap.Close()
	...
	app := httpTransport.NewServer(ctx, db, cfg.JWTSecret, bduSnap)
